...
long double s21_pow(double base, double exp) {
  ...
  if ((base == -1) && (exp * 0 != 0))
    return 1.0;
  if ((s21_fabs(base) < 1) && (exp * 0 != 0) && (exp < 0))
    return S21_INF;
  if ((s21_fabs(base) > 1) && (exp * 0 != 0) && (exp < 0))
    return 0;
  if ((s21_fabs(base) < 1) && (exp * 0 != 0) && (exp > 0))
  ...
  if ((base * 0 != 0) && (base < 0) &&
      ...
      (exp > 0))
    return S21_INF;
  if ((base * 0 != 0) && (base > 0) && (exp < 0))
    return +0;
  if ((base * 0 != 0) && (base > 0) && (exp > 0))
    return S21_INF;

  if (exp < 0)
    return 1 / s21_pow(base, -exp);
  int negate = (base < 0) ? 1 : 0;
  if (negate)
    base = -base;

  long double result = s21_exp(exp * s21_log(base));
  if (negate && (s21_fmod(exp, 2.0) == 1))
    result *= -1.0;
...
long double
s21_exp(double x) { // exp(x) = 1 + x + x^2/2! + x^3/3! + x^4/4! + ... + x^n/n!
  if (x != x)
    return S21_NAN;
  if ((x * 0 != 0) && (x > 0))
    return x;
